<template>
  <ul class="pagination justify-content-center">
    ...
    <li class="page-item" v-if="prev">
      <router-link :to="`/board/list?no=${ (startPageIndex - 1) * listRowCount }`" class="page-link" aria-label="Previous" @click.native="movePage2(startPageIndex - 1)">
    ...
    <li v-for="index in endPageIndex-startPageIndex + 1 " :key="index" class="page-item" :class="{active:( (startPageIndex + index - 1) == currentPageIndex)}">
      <router-link :to="`/board/list?no=${ (startPageIndex + index - 1) * listRowCount }`" class="page-link"  @click.native="movePage2(startPageIndex + index - 1)">{{ startPageIndex + index - 1 }}</router-link>
      <!-- <a class="page-link" href="javascript:movePage(' + i + ')">' + i + '</a> -->
    </li>

    <li class="page-item" v-if="next">
      <router-link :to="`/board/list?no=${ (endPageIndex + 1) * listRowCount }`" class="page-link" aria-label="Next"  @click.native="movePage2(endPageIndex + 1)">
